...
async def validate_method(themethod, quiet=False):
    ...
    for call_line in call_lines:
        if not await validate_docstring(method_string, call_line, quiet):
            return False
    return True


async def validate_docstring(method_string, call_line, quiet):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(docfile_URL) as html_doc_response:
            if html_doc_response.status != int(http.HTTPStatus.OK):
                if not quiet:
                    print('{} docstring URL request returned {}'.format(
                        method_string,
                        html_doc_response.status
                    ))
                return False

            endpoint_h2 = re.search(
                r'<h2[^>]*name=[\'\"]{}[\'\"]'.format(endpoint_name),
                await html_doc_response.text()
            )
            ...
            endpoint_search_end = after_endpoint_re.search(await html_doc_response.text(),
                                                        endpoint_search_start_pos)
            if not endpoint_search_end:
                endpoint_search_stop_pos = len(await html_doc_response.text())
            else:
...
async def test_methods():
    ...
    for method_to_test in methods:
        await validate_method(method_to_test)
