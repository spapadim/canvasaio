...
from canvasaio.blueprint import BlueprintMigration
from canvasaio.blueprint import ChangeRecord
...
@aioresponse_mock
class TestBlueprint(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        with aioresponses() as m:
            ...
            self.blueprint = await self.course.get_blueprint(1)
            self.blueprint_migration = await self.blueprint.show_blueprint_migration(1)
    ...
    async def test_get_associated_courses(self, m):
        ...
        self.assertEqual((await associated_courses[0]).id, 1)
        self.assertIsInstance(await associated_courses[0], Course)

    # update_associated_courses()
    async def test_update_associated_courses(self, m):
        register_uris({"blueprint": ["update_associated_courses"]}, m)
        updated_associations = await self.blueprint.update_associated_courses()
        self.assertEqual(updated_associations, True)

    # associated_course_migration()
    async def test_associated_course_migration(self, m):
        register_uris({"blueprint": ["associated_course_migration"]}, m)
        associated_migration = await self.blueprint.associated_course_migration()
    ...
    async def test_change_blueprint_restrictions(self, m):
        register_uris({"blueprint": ["change_blueprint_restrictions"]}, m)
        blueprint_restriction = await self.blueprint.change_blueprint_restrictions(
            "quiz", 1, True
        )
        self.assertIsInstance(blueprint_restriction, bool)

    # get_unsynced_changes()
    async def test_get_unsynced_changes(self, m):
        ...
        unsynced_changes_0 = await unsynced_changes[0]
        self.assertIsInstance(unsynced_changes_0, ChangeRecord)
        self.assertEqual(unsynced_changes_0.asset_id, 1)
        self.assertEqual(unsynced_changes_0.asset_type, "quiz")
        self.assertEqual(unsynced_changes_0.asset_name, "test quiz")
        self.assertEqual(unsynced_changes_0.change_type, "updated")

    # list_blueprint_migrations()
    async def test_list_blueprint_migrations(self, m):
        ...
        blueprint_migrations_0 = await blueprint_migrations[0]
        self.assertIsInstance(blueprint_migrations_0, BlueprintMigration)
        self.assertEqual(blueprint_migrations_0.id, 1)
        self.assertEqual(blueprint_migrations_0.user_id, 1)
        self.assertEqual(blueprint_migrations_0.template_id, 1)

    # show_blueprint_migration()
    async def test_show_blueprint_migration(self, m):
        register_uris({"blueprint": ["show_blueprint_migration"]}, m)
        blueprint_migration = await self.blueprint.show_blueprint_migration(1)
...
@aioresponse_mock
class TestBlueprintSubscription(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.canvas = Canvas(settings.BASE_URL, settings.API_KEY)
        ...
        with aioresponses() as m:
            ...
            self.blueprint = await self.course.get_blueprint(1)
            self.blueprint_migration = await self.blueprint.show_blueprint_migration(1)
            self.blueprint_subscription = await self.course.list_blueprint_subscriptions()[0]
    ...
    async def test_list_blueprint_imports(self, m):
        ...
        blueprint_imports_0 = await blueprint_imports[0]
        self.assertIsInstance(blueprint_imports_0, BlueprintMigration)
        self.assertEqual(blueprint_imports_0.id, 3)
        self.assertEqual(blueprint_imports_0.subscription_id, 10)

        # show_blueprint_import

    async def test_show_blueprint_import(self, m):
        register_uris({"blueprint": ["show_blueprint_import"]}, m)
        blueprint_import = await self.blueprint_subscription.show_blueprint_import(3)
        self.assertIsInstance(blueprint_import, BlueprintMigration)


@aioresponse_mock
class TestBlueprintMigration(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.canvas = Canvas(settings.BASE_URL, settings.API_KEY)
        ...
        with aioresponses() as m:
            ...
            self.blueprint_imports = (
                await self.blueprint_subscription.list_blueprint_imports()[0]
            )
            self.b_import = await self.blueprint_subscription.show_blueprint_import(3)
    ...
    # get_details()
    async def test_get_details(self, m):
        ...
        migration_details = [detail async for detail in migration_details]
    ...
    # get_import_details()
    async def test_get_import_details(self, m):
        ...
        self.assertIsInstance(await import_details[0], ChangeRecord)


@aioresponse_mock
class TestChangeRecord(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.canvas = Canvas(settings.BASE_URL, settings.API_KEY)
        ...
        with aioresponses() as m:
            ...
            self.course = await self.canvas.get_course(1)
            self.blueprint = await self.course.get_blueprint(1)
            self.change_record = await self.blueprint.get_unsynced_changes()[0]
