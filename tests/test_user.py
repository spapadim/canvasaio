...
class TestUser(unittest.IsolatedAsyncioTestCase):
    ...
    # get_profile()
    async def test_get_profile(self, m):
    ...
    # get_page_views()
    async def test_get_page_views(self, m):
        register_uris({"user": ["page_views", "page_views_p2"]}, m)

        page_views = self.user.get_page_views()
        page_view_list = [view async for view in page_views]
    ...
    async def test_get_missing_submissions(self, m):
        register_uris({"user": ["missing_sub", "missing_sub_p2"]}, m)

        missing_assigments = self.user.get_missing_submissions()
        assignment_list = [assignment async for assignment in missing_assigments]
    ...
    async def test_update_settings(self, m):
        ...
        settings = await self.user.update_settings(manual_mark_as_read=True)
    ...
    # get_color()
    async def test_get_color(self, m):
        register_uris({"user": ["color"]}, m)

        color = await self.user.get_color("course_1")
    ...
    # get_colors()
    async def test_get_colors(self, m):
        register_uris({"user": ["colors"]}, m)

        colors = await self.user.get_colors()
    ...
    # update_color()
    async def test_update_color(self, m):
        register_uris({"user": ["color_update"]}, m)

        new_hexcode = "#f00f00"
        color = await self.user.update_color("course_1", new_hexcode)

        self.assertIsInstance(color, dict)
        self.assertIn("hexcode", color)
        self.assertEqual(color["hexcode"], new_hexcode)

    async def test_update_color_no_hashtag(self, m):
        register_uris({"user": ["color_update"]}, m)
    ...
    async def test_edit(self, m):
        ...
        await self.user.edit(user={"name": new_name})
    ...
    # merge_into()
    async def test_merge_into_id(self, m):
        register_uris({"user": ["merge"]}, m)

        await self.user.merge_into(2)

        self.assertIsInstance(self.user, User)
        self.assertTrue(hasattr(self.user, "name"))
        self.assertEqual(self.user.name, "John Smith")

    async def test_merge_into_user(self, m):
        register_uris({"user": ["get_by_id_2", "merge"]}, m)

        other_user = await self.canvas.get_user(2)
        await self.user.merge_into(other_user)
    ...
    async def test_user_get_assignments(self, m):
        ...
        assignment_list = [assignment async for assignment in assignments_by_id]
        self.assertIsInstance(await assignments_by_id[0], Assignment)
        self.assertEqual(len(assignment_list), 4)

        course_obj = await self.canvas.get_course(1)
        assignments_by_obj = self.user.get_assignments(course_obj)
        assignment_list = [assignment async for assignment in assignments_by_obj]
        self.assertIsInstance(await assignments_by_obj[0], Assignment)
        self.assertEqual(len(assignment_list), 4)
    ...
    async def test_upload(self, m):
        ...
        try:
            with open(filename, "w+") as file:
                response = await self.user.upload(file)
    ...
    async def test_get_calendar_events_for_user(self, m):
    ...
    async def test_get_file(self, m):
        ...
        file_by_id = await self.user.get_file(1)
        ...
        file_by_obj = await self.user.get_file(file_by_id)
    ...
    async def test_get_folder(self, m):
        ...
        folder_by_id = await self.user.get_folder(1)
        self.assertEqual(folder_by_id.name, "Folder 1")
        self.assertIsInstance(folder_by_id, Folder)

        folder_by_obj = await self.user.get_folder(folder_by_id)
    ...
    async def test_create_folder(self, m):
        ...
        response = await self.user.create_folder(name=name_str)
    ...
    # get_observees()
    async def test_get_observees(self, m):
        requires = {"user": ["list_observees", "list_observees_2"]}
        register_uris(requires, m)

        response = self.user.get_observees()
        observees_list = [observees async for observees in response]

        self.assertIsInstance(observees_list[0], User)
        self.assertEqual(len(observees_list), 4)

    # add_observee_with_credentials()
    async def test_add_observee_with_credentials(self, m):
        register_uris({"user": ["add_observee_with_credentials"]}, m)

        response = await self.user.add_observee_with_credentials()

        self.assertIsInstance(response, User)

    # show_observee()
    async def test_show_observee(self, m):
        register_uris({"user": ["show_observee"]}, m)

        response = await self.user.show_observee(6)

        self.assertIsInstance(response, User)

    # add_observee()
    async def test_add_observee(self, m):
        register_uris({"user": ["add_observee"]}, m)

        response = await self.user.add_observee(7)

        self.assertIsInstance(response, User)

    # remove_observee()
    async def test_remove_observee(self, m):
        register_uris({"user": ["remove_observee"]}, m)

        response = await self.user.remove_observee(8)
    ...
    async def test_create_content_migration(self, m):
        ...
        content_migration = await self.user.create_content_migration("dummy_importer")
    ...
    async def test_create_content_migration_migrator(self, m):
        ...
        content_migration = await self.user.create_content_migration(await migrators[0])
    ...
    async def test_get_content_migrations(self, m):
        ...
        response = self.user.get_content_migrations()
    ...
    async def test_get_migration_systems(self, m):
        ...
        response = self.user.get_migration_systems()
    ...
    async def test_show_content_export(self, m):
        ...
        content_export = await self.user.get_content_export(11)
    ...
    async def test_export_content(self, m):
        ...
        content_export = await self.user.export_content("d")
    ...
    async def test_get_feature_flag(self, m):
        ...
        feature = await self.user.get_features()[0]

        feature_flag = await self.user.get_feature_flag(feature)
    ...
    async def test_set_usage_rights(self, m):
        ...
        usage_rights = await self.user.set_usage_rights(
            file_ids=[1, 2],
    ...
    async def test_remove_usage_rights(self, m):
        ...
        retval = await self.user.remove_usage_rights(file_ids=[1, 2])
...
@aioresponse_mock
class TestUserDisplay(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.canvas = Canvas(settings.BASE_URL, settings.API_KEY)
        ...
        with aioresponses() as m:
            ...
            self.userDisplayList = [ud async for ud in self.userDisplays]
