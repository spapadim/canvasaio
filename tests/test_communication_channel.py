...
from canvasaio.notification_preference import NotificationPreference
from tests import settings
from tests.util import register_uris, aioresponse_mock


@aioresponse_mock
class TestCommunicationChannel(unittest.IsolatedAsyncioTestCase):
    ...
    # get_preferences()
    async def test_get_preferences(self, m):
        register_uris({"communication_channel": ["list_preferences"]}, m)

        preferences = await self.comm_chan.get_preferences()
    ...
    async def test_get_preference_categories(self, m):
        register_uris({"communication_channel": ["list_preference_categories"]}, m)

        categories = await self.comm_chan.get_preference_categories()
    ...
    # get_preference()
    async def test_get_preference(self, m):
        register_uris({"communication_channel": ["get_preference"]}, m)

        preference = await self.comm_chan.get_preference("new_announcement")
    ...
    # update_preference()
    async def test_update_preference(self, m):
        register_uris({"communication_channel": ["update_preference"]}, m)
        notification = "new_announcement"
        frequency = "daily"

        updated_pref = await self.comm_chan.update_preference(
    ...
    async def test_update_preferences_by_category(self, m):
        register_uris({"communication_channel": ["update_preferences_by_category"]}, m)
        category = "course_content"
        frequency = "daily"

        updated_prefs = await self.comm_chan.update_preferences_by_catagory(
    ...
    async def test_update_multiple_preferences(self, m):
        ...
        updated_prefs = await self.comm_chan.update_multiple_preferences(
        ...
        self.assertFalse(
            await self.comm_chan.update_multiple_preferences(
    ...
    async def test_delete(self, m):
        ...
        self.assertTrue(await new_channel.delete())
